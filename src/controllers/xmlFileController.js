...
async function parseFile(filePath, dataSource) {
    try {
        ...
        const elements = result === null || result === void 0 ? void 0 : result.Report;
...
async function processReport(report, idRpt, mag, reportDate, dataSource) {
    switch (idRpt.trim()) {
        //case 'S04':
...
async function processS04(report, mag, reportDate, dataSource) {
    var _a;
    let res = [];
    const s04Repository = dataSource.getRepository(T_S04_TEMP_1.T_S04_TEMP);
    const batchSize = 1000;
    try {
        for (const elem of (_a = report === null || report === void 0 ? void 0 : report.Cnc[0]) === null || _a === void 0 ? void 0 : _a.Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S04) && Array.isArray(elem.S04)) {
                for (let i = 0; i < Object.keys(elem.S04).length; i++) {
                    var s04 = new T_S04_TEMP_1.T_S04_TEMP();
                    s04.cnt_id = elem.$.Id;
                    s04.fh_i = (0, utils_1.parseDate)(elem.S04[i].$.Fhi);
                    s04.fh_f = (0, utils_1.parseDate)(elem.S04[i].$.Fhf);
                    s04.ctr = elem.S04[i].$.Ctr;
                    s04.pt = elem.S04[i].$.Pt;
                    s04.mx = elem.S04[i].$.Mx;
                    if ((0, utils_1.isValidDate)(elem.S04[i].$.Fx)) {
                        s04.fx = (0, utils_1.parseDate)(elem.S04[i].$.Fx);
                    }
                    else {
                        s04.fx = (0, utils_1.parseDate)('20000101000000');
                    }
...
async function processS09(report, mag, reportDate, dataSource) {
    var _a, _b;
    let res = [];
    const batchSize = 1000;
    const s09Repository = dataSource.getRepository(T_S09_TEMP_1.T_S09_TEMP);
    try {
        for (const elem of (_a = report === null || report === void 0 ? void 0 : report.Cnc[0]) === null || _a === void 0 ? void 0 : _a.Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S09) && Array.isArray(elem.S09)) {
                for (let i = 0; i < Object.keys(elem.S09).length; i++) {
                    var s09 = new T_S09_TEMP_1.T_S09_TEMP();
                    s09.cnt_id = elem.$.Id;
                    s09.fh = (0, utils_1.parseDate)(elem.S09[i].$.Fh);
                    s09.et = elem.S09[i].$.Et;
                    s09.c = elem.S09[i].$.C;
                    s09.d1 = (_b = elem === null || elem === void 0 ? void 0 : elem.S09[i]) === null || _b === void 0 ? void 0 : _b.D1;
                    res.push(s09);
                    if (res.length >= batchSize) {
...
async function processS05(report, mag, reportDate, dataSource) {
    var _a;
    ...
    try {
        for (const elem of (_a = report === null || report === void 0 ? void 0 : report.Cnc[0]) === null || _a === void 0 ? void 0 : _a.Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S05) && Array.isArray(elem.S05)) {
                for (let i = 0; i < Object.keys(elem.S05).length; i++) {
                    var s05 = new T_S05_TEMP_1.T_S05_TEMP();
                    s05.cnt_id = elem.$.Id;
                    s05.fh = (0, utils_1.parseDate)(elem.S05[i].$.Fh);
...
async function processS02(report, mag, reportDate, dataSource) {
    var _a;
    ...
    try {
        for (const elem of (_a = report === null || report === void 0 ? void 0 : report.Cnc[0]) === null || _a === void 0 ? void 0 : _a.Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S02) && Array.isArray(elem.S02)) {
                for (let i = 0; i < Object.keys(elem.S02).length; i++) {
                    var s02 = new T_S02_TEMP_1.T_S02_TEMP();
                    s02.cnt_id = elem.$.Id;
                    s02.magn = parseInt(elem.$.Magn);
                    s02.fh = (0, utils_1.parseDate)(elem.S02[i].$.Fh);
...
async function processG01(report, mag, reportDate, dataSource) {
    const g01Repository = dataSource.getRepository(T_G01_TEMP_1.T_G01_TEMP);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Cnc) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G01) && Array.isArray(elem.G01)) {
                for (let i = 0; i < Object.keys(elem.G01).length; i++) {
                    var g01 = new T_G01_TEMP_1.T_G01_TEMP();
                    g01.cnc_id = elem.$.Id;
                    g01.fh = (0, utils_1.parseDate)(elem.G01[i].$.Fh);
...
async function processG02(report, mag, reportDate, dataSource) {
    const g02Repository = dataSource.getRepository(T_G02_TEMP_1.T_G02_TEMP);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G02) && Array.isArray(elem.G02)) {
                for (let i = 0; i < Object.keys(elem.G02).length; i++) {
                    var g02 = new T_G02_TEMP_1.T_G02_TEMP();
                    g02.cnt_id = elem.$.Id;
                    g02.fh = (0, utils_1.parseDate)(elem.G02[i].$.Fh);
...
async function processG03(report, mag, reportDate, dataSource) {
    const g03Repository = dataSource.getRepository(T_G03_TEMP_1.T_G03_TEMP);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G03) && Array.isArray(elem.G03)) {
                for (let i = 0; i < Object.keys(elem.G03).length; i++) {
                    var g03 = new T_G03_TEMP_1.T_G03_TEMP;
                    g03.cnt_id = elem.$.Id;
                    g03.fh = (0, utils_1.parseDate)(elem.G03[i].$.Fh);
...
async function processG04(report, mag, reportDate, dataSource) {
    const g04Repository = dataSource.getRepository(T_G04_TEMP_1.T_G04_TEMP);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G04) && Array.isArray(elem.G04)) {
                for (let i = 0; i < Object.keys(elem.G04).length; i++) {
                    var g04 = new T_G04_TEMP_1.T_G04_TEMP();
                    g04.cnt_id = elem.$.Id;
                    g04.fh = (0, utils_1.parseDate)(elem.G04[i].$.Fh);
...
async function processG05(report, mag, reportDate, dataSource) {
    const g05Repository = dataSource.getRepository(T_G05_TEMP_1.T_G05_TEMP);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G05) && Array.isArray(elem.G05)) {
                for (let i = 0; i < Object.keys(elem.G05).length; i++) {
                    var g05 = new T_G05_TEMP_1.T_G05_TEMP();
                    g05.cnt_id = elem.$.Id;
                    g05.fh = (0, utils_1.parseDate)(elem.G05[i].$.Fh);
...
async function processG06(report, mag, reportDate, dataSource) {
    const g06Repository = dataSource.getRepository(T_G06_TEMP_1.T_G06_TEMP);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G06) && Array.isArray(elem.G06)) {
                for (let i = 0; i < Object.keys(elem.G06).length; i++) {
                    var g06 = new T_G06_TEMP_1.T_G06_TEMP();
                    g06.cnt_id = elem.$.Id;
                    g06.fh = (0, utils_1.parseDate)(elem.G06[i].$.Fh);
...
async function processG07(report, mag, reportDate, dataSource) {
    const g07Repository = dataSource.getRepository(T_G07_TEMP_1.T_G07_TEMP);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G07) && Array.isArray(elem.G07)) {
                for (let i = 0; i < Object.keys(elem.G07).length; i++) {
                    var g07 = new T_G07_TEMP_1.T_G07_TEMP();
                    g07.cnt_id = elem.$.Id;
                    g07.fh = (0, utils_1.parseDate)(elem.G07[i].$.Fh);
...
async function processG56(report, mag, reportDate, dataSource) {
    const g56Repository = dataSource.getRepository(T_G56_1.T_G56);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G56) && Array.isArray(elem.G56)) {
                for (let i = 0; i < Object.keys(elem.G56).length; i++) {
                    var g56 = new T_G56_1.T_G56();
                    g56.rtu_id = report.Rtu[0].$.Id;
                    g56.lvs_id = elem.$.Id;
                    g56.lvs_pos = elem.$.Pos;
                    g56.fh = (0, utils_1.parseDate)(elem.G56[i].$.Fh);
...
async function processG57(report, mag, reportDate, dataSource) {
    const g57Repository = dataSource.getRepository(T_G57_1.T_G57);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G57) && Array.isArray(elem.G57)) {
                for (let i = 0; i < Object.keys(elem.G57).length; i++) {
                    var g57 = new T_G57_1.T_G57();
                    g57.rtu_id = report.Rtu[0].$.Id;
                    g57.lvs_id = elem.$.Id;
                    g57.lvs_pos = elem.$.Pos;
                    g57.fh = (0, utils_1.parseDate)(elem.G57[i].$.Fh);
...
async function processG58(report, mag, reportDate, dataSource) {
    const g58Repository = dataSource.getRepository(T_G58_1.T_G58);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G58) && Array.isArray(elem.G58)) {
                for (let i = 0; i < Object.keys(elem.G58).length; i++) {
                    var g58 = new T_G58_1.T_G58();
                    g58.rtu_id = report.Rtu[0].$.Id;
                    g58.lvs_id = elem.$.Id;
                    g58.lvs_pos = elem.$.Pos;
                    g58.fh = (0, utils_1.parseDate)(elem.G58[i].$.Fh);
...
async function processS93(report, mag, reportDate, dataSource) {
    const s93Repository = dataSource.getRepository(T_S93_1.T_S93);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S93) && Array.isArray(elem.S93)) {
                for (let i = 0; i < Object.keys(elem.S93).length; i++) {
                    var s93 = new T_S93_1.T_S93();
                    s93.rtu_id = elem.$.Id;
                    s93.fh = (0, utils_1.parseDate)(elem.S93[i].$.Fh);
...
async function processS94(report, mag, reportDate, dataSource) {
    const s94Repository = dataSource.getRepository(T_S94_1.T_S94);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S94) && Array.isArray(elem.S94)) {
                for (let i = 0; i < Object.keys(elem.S94).length; i++) {
                    var s94 = new T_S94_1.T_S94();
                    s94.rtu_id = elem.$.Id;
                    s94.tp = elem.S94[i].$.Tp;
                    s94.fh = (0, utils_1.parseDate)(elem.S94[i].$.Fh);
...
async function processS96(report, mag, reportDate, dataSource) {
    const s96Repository = dataSource.getRepository(T_S96_1.T_S96);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S96) && Array.isArray(elem.S96)) {
                for (let i = 0; i < Object.keys(elem.S96).length; i++) {
                    var s96 = new T_S96_1.T_S96;
                    s96.rtu_id = elem.$.Id;
                    s96.fh = (0, utils_1.parseDate)(elem.S96[i].$.Fh);
...
async function processS97(report, mag, reportDate, dataSource) {
    const s97Repository = dataSource.getRepository(T_S97_1.T_S97);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S97) && Array.isArray(elem.S97)) {
                for (let i = 0; i < Object.keys(elem.S97).length; i++) {
                    var s97 = new T_S97_1.T_S97();
                    s97.rtu_id = elem.$.Id;
                    s97.fh = (0, utils_1.parseDate)(elem.S97[i].$.Fh);
...
async function processS06(report, mag, reportDate, dataSource) {
    const s06Repository = dataSource.getRepository(T_S06_1.T_S06);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S06) && Array.isArray(elem.S06)) {
                for (let i = 0; i < Object.keys(elem.S06).length; i++) {
                    var s06 = new T_S06_1.T_S06();
                    s06.cnt_id = report.Cnc[0].$.Id;
                    s06.cnc_id = elem.$.Id;
                    s06.fh = (0, utils_1.parseDate)(elem.S06[i].$.Fh);
...
async function processS12(report, mag, reportDate, dataSource) {
    const s12Repository = dataSource.getRepository(T_S12_1.T_S12);
    const elem = report.Cnc[0];
    try {
        var s12 = new T_S12_1.T_S12();
        s12.cnc_id = elem.$.Id;
        s12.fh = (0, utils_1.parseDate)(elem.S12[0].$.Fh);
...
async function processS14(report, mag, reportDate, dataSource) {
    const s14Repository = dataSource.getRepository(T_S14_1.T_S14);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report.Cnc[0].Cnt) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S14) && Array.isArray(elem.S14)) {
                for (let i = 0; i < Object.keys(elem.S14).length; i++) {
                    var s14 = new T_S14_1.T_S14();
                    s14.cnc_id = report.Cnc[0].$.Id;
                    s14.cnt_id = elem.$.Id;
                    s14.fh = (0, utils_1.parseDate)(elem.S14[i].$.Fh);
...
async function processS17(report, mag, reportDate, dataSource) {
    const s17Repository = dataSource.getRepository(T_S17_1.T_S17);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report.Cnc) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S17) && Array.isArray(elem.S17)) {
                for (let i = 0; i < Object.keys(elem.S17).length; i++) {
                    var s17 = new T_S17_1.T_S17();
                    s17.cnc_id = elem.$.Id;
                    s17.fh = (0, utils_1.parseDate)(elem.S17[i].$.Fh);
...
async function processS24(report, mag, reportDate, dataSource) {
    const S24Repository = dataSource.getRepository(T_S24_1.T_S24);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report.Cnc[0].S24[0].Meter) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S24) && Array.isArray(elem.S24)) {
                for (let i = 0; i < Object.keys(elem).length; i++) {
                    var S24 = new T_S24_1.T_S24();
                    S24.cnc_id = report.Cnc[0].$.Id;
                    S24.fh = (0, utils_1.parseDate)(report.Cnc[0].S24[0].$.Fh);
                    S24.meter_id = elem.$.MeterId;
                    S24.comstatus = elem.$.ComStatus;
                    S24.date = (0, utils_1.parseDate)(elem.$.Date);
...
async function processG59(report, mag, reportDate, dataSource) {
    const G59Repository = dataSource.getRepository(T_G59_1.T_G59);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G59) && Array.isArray(elem.G59)) {
                for (let i = 0; i < Object.keys(elem.G59).length; i++) {
                    var G59 = new T_G59_1.T_G59();
                    G59.rtu_id = report.Rtu[0].$.Id;
                    G59.lvs_id = elem.$.Id;
                    G59.lvs_pos = elem.$.Pos;
                    G59.fh = (0, utils_1.parseDate)(elem.G59[i].$.Fh);
...
async function processS52(report, mag, reportDate, dataSource) {
    const S52Repository = dataSource.getRepository(T_S52_1.T_S52);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S52) && Array.isArray(elem.S52)) {
                for (let i = 0; i < Object.keys(elem.S52).length; i++) {
                    const S52 = new T_S52_1.T_S52();
                    S52.rtu_id = report.Rtu[0].$.Id;
                    S52.lvs_id = elem.$.Id;
                    S52.lvs_pos = elem.$.Pos;
                    S52.lvs_magn = elem.$.Magn;
                    // Procesar la fecha y hora de fin
                    const fhFin = (0, utils_1.parseDate)(elem.S52[i].$.Fh); // Esta función debe devolverte un objeto Date
...
async function processS53(report, mag, reportDate, dataSource) {
    const S53Repository = dataSource.getRepository(T_S53_1.T_S53);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S53) && Array.isArray(elem.S53)) {
                for (let i = 0; i < Object.keys(elem.S53).length; i++) {
                    var S53 = new T_S53_1.T_S53();
                    S53.rtu_id = report.Rtu[0].$.Id;
                    S53.lvs_id = elem.$.Id;
                    S53.lvs_pos = elem.$.Pos;
                    S53.lvs_magn = elem.$.Magn;
                    const fhFin = (0, utils_1.parseDate)(elem.S53[i].$.Fh);
...
async function processS59(report, mag, reportDate, dataSource) {
    const S59Repository = dataSource.getRepository(T_S59_1.T_S59);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S59) && Array.isArray(elem.S59)) {
                for (let i = 0; i < Object.keys(elem.S59).length; i++) {
                    var S59 = new T_S59_1.T_S59();
                    S59.rtu_id = report.Rtu[0].$.Id;
                    S59.lvs_id = elem.$.Id;
                    S59.lvs_pos = elem.$.Pos;
                    S59.fh = (0, utils_1.parseDate)(elem.S59[i].$.Fh);
...
async function processS64(report, mag, reportDate, dataSource) {
    const S64Repository = dataSource.getRepository(T_S64_1.T_S64);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S64) && Array.isArray(elem.S64)) {
                for (let i = 0; i < Object.keys(elem.S64).length; i++) {
                    var S64 = new T_S64_1.T_S64();
                    S64.rtu_id = report.Rtu[0].$.Id;
                    S64.lvs_id = elem.$.Id;
                    S64.lvs_pos = elem.$.Pos;
                    S64.fh = (0, utils_1.parseDate)(elem.S64[i].$.Fh);
...
async function processS82(report, mag, reportDate, dataSource) {
    const S82Repository = dataSource.getRepository(T_S82_1.T_S82);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu[0].LVSLine) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S82) && Array.isArray(elem.S82)) {
                for (let i = 0; i < Object.keys(elem.S82).length; i++) {
                    var S82 = new T_S82_1.T_S82();
                    S82.rtu_id = report.Rtu[0].$.Id;
                    S82.lvs_id = elem.$.Id;
                    S82.lvs_pos = elem.$.Pos;
                    S82.fh = (0, utils_1.parseDate)(elem.S82[i].$.Fh);
                    S82.i1max_fh = (0, utils_1.parseDate)(elem.S82[i].I1max[0].$.Fh);
                    ...
                    S82.i2max_fh = (0, utils_1.parseDate)(elem.S82[i].I2max[0].$.Fh);
                    ...
                    S82.i3max_fh = (0, utils_1.parseDate)(elem.S82[i].I3max[0].$.Fh);
                    ...
                    S82.inmax_fh = (0, utils_1.parseDate)(elem.S82[i].Inmax[0].$.Fh);
                    ...
                    S82.v1max_fh = (0, utils_1.parseDate)(elem.S82[i].V1max[0].$.Fh);
                    ...
                    S82.v2max_fh = (0, utils_1.parseDate)(elem.S82[i].V2max[0].$.Fh);
                    ...
                    S82.v3max_fh = (0, utils_1.parseDate)(elem.S82[i].V3max[0].$.Fh);
...
async function processS98(report, mag, reportDate, dataSource) {
    const S98Repository = dataSource.getRepository(T_S98_1.T_S98);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S98) && Array.isArray(elem.S98)) {
                for (let i = 0; i < Object.keys(elem.S98).length; i++) {
                    var S98 = new T_S98_1.T_S98();
                    S98.rtu_id = elem.$.Id;
                    S98.fh = (0, utils_1.parseDate)(elem.S98[i].$.Fh);
...
async function processS95(report, mag, reportDate, dataSource) {
    const S95Repository = dataSource.getRepository(T_S95_1.T_S95);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S95) && Array.isArray(elem.S95)) {
                for (let i = 0; i < Object.keys(elem.S95).length; i++) {
                    var S95 = new T_S95_1.T_S95();
                    S95.rtu_id = elem.$.Id;
                    S95.fh = (0, utils_1.parseDate)(elem.S95[i].$.Fh);
...
async function processS67(report, mag, reportDate, dataSource) {
    const S67Repository = dataSource.getRepository(T_S67_1.T_S67);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S67) && Array.isArray(elem.S67)) {
                for (let i = 0; i < Object.keys(elem.S67).length; i++) {
                    var S67 = new T_S67_1.T_S67();
                    S67.rtu_id = elem.$.Id;
                    S67.fh = (0, utils_1.parseDate)(elem.S67[i].$.Fh);
...
async function processG53(report, mag, reportDate, dataSource) {
    const G53Repository = dataSource.getRepository(T_G53_1.T_G53);
    let res = [];
    const batchSize = 1000;
    try {
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.G53) && Array.isArray(elem.G53)) {
                for (let i = 0; i < Object.keys(elem.G53).length; i++) {
                    var G53 = new T_G53_1.T_G53();
                    G53.rtu_id = elem.$.Id;
                    G53.fh = (0, utils_1.parseDate)(elem.G53[i].$.Fh);
...
async function processS62(report, mag, reportDate, dataSource) {
    try {
        const S62Repository = dataSource.getRepository(T_S62_1.T_S62);
        const insertQueryBuilder = S62Repository.createQueryBuilder().insert().orIgnore();
        for (const elem of report === null || report === void 0 ? void 0 : report.Rtu) {
            if ((elem === null || elem === void 0 ? void 0 : elem.S62) && elem.S62.length > 0) {
